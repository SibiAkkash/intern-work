...
class VisualInspector:
    ...
    def handle_cycle_end(self, frame_num: float):
        # if there was any step, do necessary processing
        if self.state["prev_step"] != -1:
            self.handle_state_change(frame_num=frame_num)
    ...
    def get_step_number(self, detections: List[int]):
    ...
    def process_detections(self, detections: List[int], frame_num: int):
        if not self.cycle_started():
            ...
            if start_marker_found:
                self.handle_cycle_start(frame_num)
        ...
        if end_marker_found:
            self.handle_cycle_end(frame_num)
            return
        ...
        next_step_num, next_object_id = self.get_step_number(detections)
